...
@property (nonatomic, strong) UILabel *scanTitle;
...
-(id)initWithFrame:(CGRect)frame style:(LBXScanViewStyle*)style
{
    if (self = [super initWithFrame:frame])
    {
        ...
        self.scanTitle.text = @"将二维码放入框内,即可自动扫描";
    }
...
- (UILabel *)scanTitle
{
    if (!_scanTitle) {
        ...
        _scanTitle.backgroundColor = kColorBlackColor;
        _scanTitle.textColor = kColorWhiteColor;
        _scanTitle.font = kLabelSize13;
        _scanTitle.textAlignment = NSTextAlignmentCenter;
        _scanTitle.layer.cornerRadius = 5;
        _scanTitle.layer.masksToBounds = YES;
        [self addSubview:_scanTitle];
        [_scanTitle mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.mas_equalTo(self.mas_centerY).with.offset(WS(120));
            make.width.mas_equalTo(WS(250));
