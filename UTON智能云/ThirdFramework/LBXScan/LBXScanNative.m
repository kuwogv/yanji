...
- (void)captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    NSString *isShow = @"1";
//    CCBLogDebug(@"------------->>>>>>>>%f",brightnessValue);
    if (brightnessValue < 0) {
        NSNotification *registerSuccNoti = [NSNotification notificationWithName:@"brightnessCallBlack" object:isShow];
        [[NSNotificationCenter defaultCenter] postNotification:registerSuccNoti];
    } else {
        isShow = @"0";
        NSNotification *registerSuccNoti = [NSNotification notificationWithName:@"brightnessCallBlack" object:isShow];
        [[NSNotificationCenter defaultCenter] postNotification:registerSuccNoti];
