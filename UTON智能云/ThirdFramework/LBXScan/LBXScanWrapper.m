...
+(void) recognizeImage:(UIImage *)image success:(void (^)(NSArray<LBXScanResult *> *))block;
{
    ...
    if (features.count > 0) {
        ...
        if(block){
        LBXScanResult *result = [[LBXScanResult alloc] initWithScanString:scannedResult imgScan:tmpImg barCodeType:[LBXScanWrapper convertZXBarcodeFormat:kBarcodeFormatQRCode]];
            block(@[result]);
        }
    }
    else{
        if(block){
            LBXScanResult *result = [[LBXScanResult alloc] initWithScanString:nil imgScan:tmpImg barCodeType:[LBXScanWrapper convertZXBarcodeFormat:kBarcodeFormatQRCode]];
            block(@[result]);
...
{
    return [LBXScanWrapper generateBarCode:str size:size color:nil backGroundColor:nil];
}
