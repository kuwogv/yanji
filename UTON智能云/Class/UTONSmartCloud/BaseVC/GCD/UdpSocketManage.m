...
- (void)socketConnectHost
{
    ...
    [self.reciveSocket bindToPort:1900 error:nil];
    ...
    [self.sendSocket connectToHost:self.socketHost onPort:1900 error:nil];
...
{
    NSString *heartBeat = @"heshu-smartcloud";
    NSData *sendData = [heartBeat dataUsingEncoding:NSUTF8StringEncoding];
    if ([self.sendSocket sendData:sendData toHost:@"239.255.255.255" port:1900 withTimeout:-1 tag:1]) {
        NSLog(@"发送指令成功");
    }else {
        NSLog(@"发送指令失败");
    }
...
- (BOOL)onUdpSocket:(AsyncUdpSocket *)sock didReceiveData:(NSData *)data withTag:(long)tag fromHost:(NSString *)host port:(UInt16)port
{
    ...
    NSLog(@"收到的数据：%@",reciveStr);

    [self.sendSocket receiveWithTimeout:-1 tag:1];
    [self.reciveSocket receiveWithTimeout:-1 tag:0];
    //保存设备数据
//    NSData *jsonData = [reciveStr dataUsingEncoding:NSUTF8StringEncoding];
//    NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:jsonData
//                                                        options:NSJSONReadingMutableContainers
//                                                          error:nil];
//    if (dic) {
//        NSString *deviceId = dic[@"device_id"];
//        [[NSUserDefaults standardUserDefaults] setValue:dic forKey:deviceId];
