...
- (NSString *)aesEncryptWithHexKey:(NSString *)key hexIv:(NSString *)iv {
    ...
    NSString *baseStr_GTM = [self encodeBase64Data:resultData];
    return baseStr_GTM;//[resultData base64EncodedStringWithOptions:0];
}
...
//    NSString *baseStr_GTM = [self encodeBase64Data:resultData];
//    return baseStr_GTM;
    return [resultData base64EncodedStringWithOptions:5];
}
...
#pragma mark - AES解密
- (NSString *)aesBase64StringDecryptWithHexKey:(NSString *)key iv:(NSString *)iv {
    NSData *aesKey = [key dataUsingEncoding:NSUTF8StringEncoding];
    if (iv == nil) {
        // 32长度
        iv = @"00000000000000000000000000000000";
...
/*
 ECB模式解密，要求输入为Base64格式，返回为NSString
 */
- (NSString *)aesECBBase64StringDecryptWithKey:(NSString *)key {
    NSData *aesKey = [key dataUsingEncoding:NSUTF8StringEncoding];
    NSData *data = [[NSData alloc] initWithBase64EncodedString:self options:0];
    NSData *resultData = [NSString aesECBDecryptWithData:data withDataKey:aesKey];
    return [[NSString alloc] initWithData:resultData encoding:NSUTF8StringEncoding];
